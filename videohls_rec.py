...
def face_rec():
    ...
    [X,y] = read_images(sys.argv[1])#读取图片
    y = np.asarray(y, dtype=np.int32)
    
    if len(sys.argv) == 3:
        out_dir = sys.argv[2]
    
    model = cv2.face.LBPHFaceRecognizer_create()#调用模型
    model.train(np.asarray(X), np.asarray(y))
    camera = cv2.VideoCapture("2.mp4")#加载视频
    #camera = cv2.VideoCapture(0)

    face_cascade = cv2.CascadeClassifier('./cascades/haarcascade_frontalface_alt2.xml')#加载模型
    while (True):
      ...
      for (x, y, w, h) in faces:
        ...
        try:
            ...
            global image = ft.draw_text(img, pos, names[params[0]], text_size, color)#中文输出
